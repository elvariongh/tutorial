...
function Game() {
    ...
    this.screen = new TiledGameEngine.ViewPort('#viewport', window.innerWidth, 480);//window.innerWidth, window.innerHeight);
...
//    this.map.addEntity(this.path);
    ...
    this.map.moveEntity(this.sel, 25, 15);
...
//                    this.map.setEntityPath(obj, p);
                ...
                // } break;
                case 39 /* Right */: {
                    // if (obj.x == this.map.width-1) break;
                    
                    // var p = this.map.getPath(obj.x+obj.dx, obj.y+obj.dy, obj.x+2, obj.y, true);
