...
(function(TGE) {
	...
	TiledMap.prototype['reset'] = function(assetsManager, asset) {
        this['asset'] = asset;
...
//        }
        ...
        // load gears for map, if specified
        if (json['properties'] && json['properties']['gears']) {
			var s = json['properties']['gears'].split(',');
            this['assetsManager']['request'](s[1].trim());
            this['assetsManager']['download'](this.onGearsLoaded.bind(this));
        }
...
//        this.onGearsLoaded();
    ...
    TiledMap.prototype.onGearsLoaded = function() {
        var json = this['assetsManager']['get'](this['asset']);
        
        if (json['properties'] && json['properties']['gears']) {
			var s = json['properties']['gears'].split(',');
			if (TGE['gears']) {
				this['gears'] = TGE['gears'][s[0].trim()];//this['assetsManager']['get'](json['properties']['gears']);
			}
        }
...
//		console.log('sortEntities');
        ...
        for (; i < l; ++i) {
            ...
            if (obj['x'] + obj['dx'] === x && obj['y'] + obj['dy'] === y) {
                if (clickable && obj['clickable']) {
					...
					if (obj['properties']['onClick']) {
						console.log('onClick trigger', obj['properties']['onClick']);
						this._trigger(obj, ['onClick'], obj);
					}
				}
            }
        }
	...
	TiledMap.prototype['getEntityByName'] = function(name) {
		if (!name) return undefined;
    ...
    TiledMap.prototype['getEntityById2'] = function(id) {
        for (var i = 0, l = this['entities'].length; i < l; ++i) {
            var ent = this['entities'][i];
...
//        this['_entitiesMap'][ent['x'] + ent['y']*this.width] = ent['id'];
        ...
        for (; i < l; ++i) {
            ...
            if (ent.x === x     	&& (ent.y >= y && ent.y < y + h) ||
                ent.x === x + w - 1 && (ent.y >= y && ent.y < y + h) ||
                ent.y === y     	&& (ent.x >= x && ent.x < x + w) ||
                ent.y === y + h - 1 && (ent.x >= x && ent.x < x + w))
            {
				console.log('mapTrespassing', obj);
				
				if ((ent.x + ent.dx < x) ||			// w
					(ent.x + ent.dx > x + w - 1) ||	// e
					(ent.y + ent.dy < y) || 		// n
					(ent.y + ent.dy > y + h - 1)) {	// s
				
                    // entity step inside object borders - notify listeners
                    TGE['bus']['notify']('mapTrespassing', {entity: ent, object: obj});

					if (obj['properties']) {
						this._trigger(obj, ['onEnter'], ent);
/*						var triggers = ['onEnter'];
						
						for (var j = 0, c = triggers.length; j < c; ++j) {
							if (obj['properties'][triggers[j]]) {
								var cmds = obj['properties'][triggers[j]].split(';');
								for (var k = 0, s = cmds.length; k < s; ++k) {
									var cmd = cmds[k].trim();
									if (!cmd.length) continue;
									
									cmd = cmds[k].trim().split('(');
									
									if (cmd.length > 1) {
										cmd[1] = cmd[1].split(')')[0].trim().split(',');
										
										for (var i = 0, l = cmd[1].length; i < l; ++i) {
											cmd[1][i] = cmd[1][i].trim();
										}
									}
									...
									if (cmd[0][0] !== '@') {
										TGE['bus']['notify']('mapTrigger', {cmd: cmd[0], args: cmd[1]});
										console.log('mapTrigger', cmd[0]);
									} else {
										cmd[0] = cmd[0].replace('@', '');
									}
									...
									if (this['gears']) {
										if (this.gears[cmd[0]]) {
											this.gears[cmd[0]](this, obj, ent, cmd[1]);
										} else {
											console.warn('No handler found in gears for', cmd[0]);
										}
									}
								}
							}
	...
	TiledMap.prototype._trigger = function(obj, triggers, ent) {
//		var triggers = ['onEnter'];
