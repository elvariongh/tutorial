...
//    var tx, ty, i;  
    ...
    function TiledMapStage(assetManager, screen, map) {
        ...
        this.cached = false;
        this.viewportMoved = 2;
        this.viewportResized = 2;
    };
    ...
    TiledMapStage.prototype['update'] = function(dt, t) {
        ...
        if (!cnt || !this.animation) {
			...
			var vpchange = this.viewportMoved === 2 || this.viewportResized === 2;
            ...
            for (var i = 0; i < cnt; ++i) {
                ...
                if (obj['mutable']) {
                    ...
                    this['redraw'] |= (obj['redraw'] && obj['visible']);
                } else if (vpchange) {
                    obj['isVisible'](vp);
                }
            }
			...
			if (vpchange) {
				this.cached = false;
				this['redraw'] = true;
				if (this.viewportMoved === 2)
					this.viewportMoved = 1;
				if (this.viewportResized === 2)
					this.viewportResized = 1;
			}
    ...
    TiledMapStage.prototype['render'] = function() {
		...
		if (this.viewportMoved === 1)
			--this.viewportMoved;
		this.cached = true;
		if (this.viewportResized === 1)
			--this.viewportResized;
        
		layerscount = this.map['entities'].length;
...
//			var ___idx = 0;
            for (i = 0; i < layerscount; ++i) {
                ...
                if (obj['visible']) {
					...
					// if (obj._type === 'ui.gui.quests') {
						// console.log(obj.z);
					// } else if (obj._type === 'tile') {
						// ctx.fillStyle = "rgba(255, 255, 255, 1)";
						// ctx.fillText(___idx++, obj.x + vp[0], obj.y + vp[1] + obj.height-12);
						// ctx.fillText(obj.z, obj.x + vp[0], obj.y + vp[1] + obj.height);
					// // } else {
						// // obj['render'](ctx, this, vp);
					// }
				}
    ...
    TiledMapStage.prototype.renderLayer = function(ctx, layerID, isox, isoy, width, height, maxx, maxy) {
		...
		for (y = startY; y <= lastY; y++) {
            for (x = startX; x <= lastX; x++) {
				...
				// ctx.fillStyle = "rgba(255, 255, 255, 1)";
				// ctx.fillText(___idx++, scr[j*5+3] + this.map['mapwidth']/2 + vp[0] + tileset['tilewidth']/2, scr[j*5+4] + vp[1] + tileset['tileheight']/2);
            }
        }
    };
...
//        ctx.textBaseline = "middle";
        ...
        for (var x = -w; x < w; x++) {
            for (var y = -h; y < h; y++) {
				...
				// ctx.fillText(idx++, scrx + dx, scry + dy + this.map.tileheight/2);
            }
        }
