...
(function(w, TGE) {
    ...
    LoadingStage.prototype['setStage'] = function(stage) {
        this.progress /= (this.currentStage / this.stages);

        this.currentStage = Math.min(stage, this.stages);

        this.progress = ~~(this.progress * this.currentStage / this.stages);
		
//		console.log('setStage', stage, this.status['total'], this.stages, this.currentStage, this.progress);
...
//		console.log('LoadingStage::start', this.status['total']);
...
//        TGE.bus.notify('invalidateStage', this.name);
    ...
    LoadingStage.prototype.fnProgress = function(name, p) {
        ...
        this.progress = ~~(this.progress / this.status['total']) * (this.currentStage / this.stages);

//		if (p === 100) {
//			console.log('fnProgress', name, this.status['total'], this.stages, this.currentStage, this.progress);
//		}
